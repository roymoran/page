...
test:
	cd src/tests/unit && go test
	cd src/tests/integration && go test
	cd src/tests/system && go test

release: $(BUILD_RELEASE_PATH) page_darwin_amd64.tar.bz2 page_darwin_arm64.tar.bz2 page_linux_amd64.tar.bz2 page_linux_arm64.tar.bz2 page_linux_arm.tar.bz2 page_windows_amd64.tar.bz2
...
page_darwin_arm64.tar.bz2:
	cd src && env GOOS=darwin GOARCH=arm64 go build -o page && tar -cjvf "page_darwin_arm64.tar.bz2" page && mv page_darwin_arm64.tar.bz2 ../$(BUILD_RELEASE_PATH) && rm page
...
# windows arm 64-bit
page_windows_arm64.tar.bz2:
	cd src && env GOOS=windows GOARCH=arm64 go build -o page.exe && tar -cjvf "page_windows_arm64.tar.bz2" page.exe && mv page_windows_arm64.tar.bz2 ../$(BUILD_RELEASE_PATH) && rm page.exe
