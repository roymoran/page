...
func (n New) UsageInfoExpanded() string {
	...
	extendedUsage += fmt.Sprintln("creates a new page.yml definition file using the default registrar and host provider that have been configured using the conf command. If a default registrar or host have not been configured, the 'page' registrar/host will be the default. The page.yml definition file contains the minimum fields required to create a new page. By default, the page.yml is created in the directory where the command is executed.")
	extendedUsage += fmt.Sprintln()
